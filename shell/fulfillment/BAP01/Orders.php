...
//        $orders[] = Mage::getModel('sales/order')->load(100024149, 'increment_id');
        return $orders;
    }
...
//        $inventory['TRA'] = array("8LTC7K"=>10);
    ...
    public function checkInventory($items,$vendorInfo,$allVendorsInventory,$skipVendor = ''){
        $MTR_ItemArray = array();
        $PAT_ItemArray = array();
        $TRA_ItemArray = array();
        $KEY_ItemArray = array();
        ...
        foreach($items as $item){
            ...
            foreach($vendorPriority as $vendorCode){
                ...
                if(!empty($skipVendor)){
                    ...
                    $skip = 0;
                    foreach($skipVendor[$vendorCode] as $obj){
    ...
    public function callPlaceOrderProcess($order,$items,$itemsArray,$vendorInfo,$allVendorsInventory,$skipVendor = ''){
        ...
        if($paymentTitle == "eBay/Amazon Payment"){
            ...
            if(count($items) == (count($MTR_items) + count($PAT_items) + count($TRA_items) + count($KEY_items)
                + count($F01_items) + count($F02_items) + count($F07_items) + count($F12_items) + count($F24_items) + count($FMP_items) + count($F50_items)
            )){

                $stateToChange = $this->_placeOrder($order,$itemsArray,$vendorInfo,$allVendorsInventory,$items,$skipVendor);
        ...
        elseif($order->hasInvoices()){
            ...
            if(count($items) == (count($MTR_items) + count($PAT_items) + count($TRA_items) + count($KEY_items)
                + count($F01_items) + count($F02_items) + count($F07_items) + count($F12_items)+ count($F24_items) + count($F50_items)+ count($FMP_items)
            )){
                ...
                if($stateToChange == "on_hold"){
                    ...
                    $logContent = "Try place order from next vendor\n";
                    echo $logContent;file_put_contents($this->logPath,$logContent, FILE_APPEND);

                }else{

                    $logContent = "Wait for PayPal to authorize.\r\n";
                    echo $logContent;file_put_contents($this->logPath,$logContent, FILE_APPEND);
    ...
    public function _placeOrder($order,$itemsArray,$vendorInfo,$allVendorsInventory,$items,$skipVendor){
...
//        F01
        if(count($F01_items) > 0){
            ...
            $FMP_orderDetail = $this->call_FMP_APIs($FMP,$order,$F01_items,$vendorInfo,'F01',1);
            if($FMP_orderDetail){
                $orderDetail['F01'] = $FMP_orderDetail;
            }
        }
        ...
        if(count($F02_items) > 0){
            ...
            $FMP_orderDetail = $this->call_FMP_APIs($FMP,$order,$F02_items,$vendorInfo,'F02',2);
            if($FMP_orderDetail){
                $orderDetail['F02'] = $FMP_orderDetail;
            }
        ...
        if(count($F07_items) > 0){
            ...
            $FMP_orderDetail = $this->call_FMP_APIs($FMP,$order,$F07_items,$vendorInfo,'F07',7);
            if($FMP_orderDetail){
                $orderDetail['F07'] = $FMP_orderDetail;
            }
        ...
        if(count($F12_items) > 0){
            ...
            $FMP_orderDetail = $this->call_FMP_APIs($FMP,$order,$F12_items,$vendorInfo,'F12',12);
            if($FMP_orderDetail){
                $orderDetail['F12'] = $FMP_orderDetail;
            }
        ...
        if(count($F24_items) > 0){
            ...
            $FMP_orderDetail = $this->call_FMP_APIs($FMP,$order,$F24_items,$vendorInfo,'F24',24);
            if($FMP_orderDetail){
                $orderDetail['F24'] = $FMP_orderDetail;
            }
        ...
        if(count($F50_items) > 0){
            ...
            $FMP_orderDetail = $this->call_FMP_APIs($FMP,$order,$F50_items,$vendorInfo,'F50',50);
            if($FMP_orderDetail){
                $orderDetail['F50'] = $FMP_orderDetail;
            }
        ...
        if(count($FMP_items) > 0){
            ...
            $FMP_orderDetail = $this->call_FMP_APIs($FMP,$order,$FMP_items,$vendorInfo,'FMP',2);
            if($FMP_orderDetail){
        ...
        if($this->isError == 0){
            ...
            $logContent = "Placing Order Failed => Tried Next vendor...\n";
            echo $logContent;file_put_contents($this->logPath,$logContent, FILE_APPEND);


            foreach($this->isError as $index=>$obj){
              $skipVendor[$index] = $obj;
            };

            $itemsArray = $this->checkInventory($items,$vendorInfo,$allVendorsInventory,$skipVendor);
...
//                $this->prepareShipment($order,$PAT_OrderDetail,$carrier,$trackingNumber);
//            }
//            else{
...
//        }
    ...
    public function call_FMP_APIs($FMP,$order,$FMP_items,$vendorInfo,$FMP_code,$warehouseCode){
        ...
        if($returnXML['status'] != "OK"){
            ...
            $this->isError = $errorArray;
            $logContent = "Error code = ".$returnXML['status']."\n";
            echo $logContent;file_put_contents($this->logPath,$logContent, FILE_APPEND);
            return 0;
        }
        else{

            $logContent = "FMP place order status = OK\n";
            echo $logContent;file_put_contents($this->logPath,$logContent, FILE_APPEND);
    ...
    public function getVendorSku($skuArray,$vendorCode){
        ...
        if(!is_array($vendorArray)){
            ...
            $message = "[BAP01] There is a temporary pause to the fulfillment process due to PMP2 server downtime. The process will resume when PMP2 is running again.\r\n\r\nThis email is auto-generated.";
    ...
    private function sendCancelEmail($message,$subject){
        ...
        if($this->envCode == 'test'){
            ...
            $mail->setToEmail(array('david@autosoez.com','jerry@autosoez.com','cs@approvedautoparts.com'));
        }

        $mail->setBody($message);
        $mail->setSubject($subject);
        $mail->setFromEmail('cs@approvedautoparts.com');
        $mail->setFromName("cs@approvedautoparts.com");
